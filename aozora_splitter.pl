#!/usr/bin/perl
# last updated : 2017/06/19 13:11:26 JST
#
# 青空文庫スプリッター
...
sub book_split {
  ...
  if ($print_verbose) { print STDERR encode($charcode, "character encode:: $charcode \n"); }
  ...
  while (my $line = <$IN>) {
	if ($line =~ m/^［＃改ページ］/) {
	  ...
	  if ( defined($OUT) ) {
		...
		print $OUT $temp;
		if ($print_verbose) {
		  print STDERR encode($charcode, "output:: $outname  \n");
		}
	  }
	  $temp = "";
	  $temp = $temp . $line;
...
# main
{
  ...
  if ($input_name) {
	if ( -f $input_name ) {
	  ...
	  my $name = $prefix_name .  "001.txt";
	  if (-f $name) {
		print encode($charcode, "$name が存在するため処理を中止します。\n");
		exit 0;
	  }
	  if ($print_verbose) {
